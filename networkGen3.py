...
def cluster_names(names, threshold=90):
    ...
    for nm in name_list:
        ...
        for other in name_list:
            ...
            if re.search(rf"{re.escape(nm)}", other) or \
                re.search(rf"{re.escape(other)}", nm):
                    cluster.add(other)
...
def build_interaction_matrix_window(text, valid_chars, window_size=3, stride=1):
    ...
    for start in range(0, n - window_size + 1, stride):
        # 윈도우에 포함된 모든 인물의 합집합
        window_chars = set().union(*sent_char_lists[start : start + window_size])
        # 같은 윈도우에 함께 있으면 상호작용 카운트
        for c1, c2 in itertools.combinations(window_chars, 2):
            matrix[c1][c2] += 1
            matrix[c2][c1] += 1
...
for idx, part in enumerate(text_parts, 1):
    ...
    matrix = build_interaction_matrix_window(resolved_all,
                                         character_set,
                                         window_size=3,
                                         stride=1)
